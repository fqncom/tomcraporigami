...
function QueryBatchOrderList() {
    ...
    {   "TransCode": "QueryBatchOrderList", 
        "BatchID": BatchID,
        "BatchStatus": E_BatchStatus_OrderDeliver
    },
    function(data) {
...
function BatchFinishCheckedOrders() {

    if (!window.confirm("确认要对选中订单进行完成处理吗？")) {
        return false;
    }
    ...
    $("#tbBatchOrderList input[type='checkbox']").each(function() {
        if (this.checked) {
            OrderID = $(this).val();
            BatchOrderFinish(OrderID);
        }
    });  
}

function BatchOrderFinish(BatchOrderID) {
